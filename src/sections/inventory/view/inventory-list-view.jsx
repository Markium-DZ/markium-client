...
export default function InventoryListView() {
  ...
  // Tabs for stock status
  const items = [
    { key: 'all', label: t('all'), match: () => true },
    {
      key: 'in_stock',
      label: t('in_stock'),
      match: (item) => !item.is_out_of_stock && !item.is_low_stock,
      color: 'success',
    },
    {
      key: 'low_stock',
      label: t('low_stock'),
      match: (item) => item.is_low_stock,
      color: 'warning',
    },
    {
      key: 'out_of_stock',
      label: t('out_of_stock'),
      match: (item) => item.is_out_of_stock,
      color: 'error',
    },
  ];
  ...
  const filterFunction = (data, filters) => {
    ...
    const item = items.find((i) => i?.key === activeTab);
