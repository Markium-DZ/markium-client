...
export default function CarsListView({ }) {
    ...
    let TABLE_HEAD = [
        ...
        { id: 'plat_number', label: t('plateNumber'), type: "two-lines-link", first: (row) => row?.plat_number, second: (row) => { }, link: (row) => { return paths.dashboard.vehicle.details(row?.id) }, width: 20 },
        { id: 'model', label: t('model'), type: "two-lines-link", first: (row) => row?.model?.translations?.name, second: (row) => row?.model?.company?.translations?.name, link: (row) => { return paths.dashboard.vehicle.details(row?.id) }, width: 60 },
        ...
        // { id: 'actions', label: t('actions'), type: "threeDots", component: (item) => <ElementActions item={item} RformulateTable={RformulateTable} setTableData={setDataFiltered} />, width: 400,  },
    ]
    ...
    useEffect(() => {
        setDataFiltered(RformulateTable(getLateCars(car,maintenance?.filter( m => m?.status?.key == "pending"))));
    }, [car, clients, contracts]);
    useEffect(() => {
        setTableData(RformulateTable(getLateCars(car,maintenance?.filter( m => m?.status?.key == "pending"))));
    }, [car, clients, contracts]);
